...
namespace Examen1PrograII
{
    internal class ClsTransacciones
    {
                    ...
                    case "1":
                        if (i < 15)
                        {
                            ...
                            switch (TipoVehiculo[i])
                            {
                                case 1:
                                    MontoPagar[i] = 500;
                                    Console.WriteLine("El monto a pagar son: " + MontoPagar[i].ToString());
                                    break;
                                case 2:
                                    MontoPagar[i] = 700;
                                    Console.WriteLine("El monto a pagar son 700 Colones." + MontoPagar[i].ToString());
                                    break;
                                case 3:
                                    MontoPagar[i] = 2.700;
                                    Console.WriteLine("El monto a pagar son  " + MontoPagar[i].ToString());
                                ...
                                case 4:
                                    MontoPagar[i] = 3.700;
                                    Console.WriteLine("El monto a pagar son 3.700 Colones." + MontoPagar[i].ToString());
                                    break;
                            }
        ...
        public void Consultar(string PlacaNum)
        {
            Console.WriteLine("Número de placa: " + PlacaNum);
            for (int i = 0; i < numPlacas.Length; i++)
            {
                if (NumPlaca[i] == PlacaNum)
                {
                    Console.WriteLine("Datos del vehículo con número de placa {0}:", numPlacas[i]);
                ...
                {
                    if (i == numPlacas.Length - 1)
                    {
                        Console.WriteLine("No se encontró número de placa {0}", numPlacas); 
                    }
        ...
        public void Modificar(string PlacaNum)
        {
            ...
            for (int i = 0; i < numPlacas.Length; i++)
            {
                if (NumPlaca[i] == PlacaNum)
                {
                    ...
                    Console.WriteLine("Ingrese el nuevo tipo de vehículo (1=Moto, 2=Vehículo Liviano, 3=Camión o Pesado, 4=Autobús):");
                    TipoVehiculo[i] = int.Parse(Console.ReadLine());
